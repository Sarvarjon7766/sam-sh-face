...
const PostUsers = () => {
	...
	const calculateLateMinutes = useCallback((checkInTime) => {
	...
	// Saralangan va filtrlangan foydalanuvchilar
	const filteredAndSortedUsers = useMemo(() => {
		...
		return filtered
	}, [users, searchTerm, selectedDepartment, selectedStatus, selectedStat, sortConfig])
	...
	const calculateStats = useCallback(() => {
		...
		const kechikkanCount = users.filter(user => user.isLate).length
						...
						<div className="grid grid-cols-2 gap-3 text-xs">
							...
							<div>
								...
								<div className="flex items-center gap-1 mt-1">
									<span className="font-medium text-black">{user.entryTime}</span>
									{user.isEdit && (!user.lastCheckInTime || (user.lastCheckInTime && user.lastCheckOutTime)) && (
										<button
											onClick={() => openTimeModal(user, 'entry')}
											className="p-1 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 transition-colors flex-shrink-0"
							...
							<div>
								...
								<div className="flex items-center gap-1 mt-1">
									<span className="font-medium text-black">{user.exitTime}</span>
	...
	// Desktop table ko'rinishi - STICKY HEADER bilan
			...
			<div className="h-full flex flex-col overflow-hidden">
				...
				<div className="flex-1 min-h-0 overflow-auto">
					<table className="w-full">
						...
						<thead className="sticky top-0 z-20 bg-gray-50">
							<tr>
								<th
									className="px-4 py-4 text-left text-sm font-medium text-gray-500 tracking-wider whitespace-nowrap w-[80px] sticky left-0 bg-gray-50 z-30 cursor-pointer hover:bg-gray-100"
									onClick={() => handleSort('hodimID')}
								>
								...
								<th
									className="px-4 py-4 text-left text-sm font-medium text-gray-500 tracking-wider whitespace-nowrap min-w-[200px] sticky left-[80px] bg-gray-50 z-20 cursor-pointer hover:bg-gray-100"
								...
								<th
									className="px-4 py-4 text-left text-sm font-medium text-gray-500 tracking-wider whitespace-nowrap min-w-[150px] cursor-pointer hover:bg-gray-100"
				...
				<div className="flex-shrink-0 bg-gray-50 px-6 py-3 border-t">
					<p className="text-sm text-gray-600">
						Jami: {filteredAndSortedUsers.length} ta xodim
					</p>
				</div>
			</div>
		)
	...
	return (
		<div className="h-screen flex flex-col w-full max-w-full overflow-hidden bg-gray-50">
			{/* Statistics section */}
			{renderStatistics()}
			...
			<div className="flex-1 min-h-0 flex flex-col">
				...
				<div className="flex-shrink-0 bg-white p-3 sm:p-4 shadow-sm">
					<div className="bg-white rounded-lg border p-3 sm:p-4">
						<div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 items-stretch">
							...
							<div className="flex flex-wrap gap-2 sm:gap-3">
								...
								<div
									...
									className="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-lg border h-full min-w-[160px] max-w-[200px] flex-shrink-0 cursor-pointer"
									title={departments.find(d => d._id === selectedDepartment)?.name || "Barcha bo'limlar"}
								>
									<FaFilter className="text-gray-500 flex-shrink-0" size={14} />
									<div className="relative w-full">
										<select
											className="appearance-none bg-transparent text-gray-800 border-none focus:outline-none focus:ring-0 text-sm w-full truncate"
										...
										>
											...
											{departments.map(dept => (
												<option
													key={dept._id}
													value={dept._id}
													title={dept.name} // hoverda to‘liq nom ko‘rinadi
												>
													{dept.name.length > 20 ? dept.name.slice(0, 100) + "..." : dept.name}
												</option>
											))}
											<option value="no-dept">Boʻlimsiz</option>
										</select>

										{/* ↓ Pastga tushuvchi belgi (dropdown icon) */}
										<FiChevronDown
											size={14}
								...
								{/* Holat filteri */}
								<div className="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-lg border h-full min-w-[130px] max-w-[160px] flex-shrink-0">
									<FaFilter className="text-gray-500 flex-shrink-0" size={14} />
									<select
								...
								<button
									...
									className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-2 rounded-lg flex items-center justify-center gap-2 text-sm h-full transition-colors duration-200 font-medium flex-shrink-0"
								>
									<FiX size={16} />
								...
								<button
									...
									disabled={refreshing}
									className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg flex items-center justify-center gap-2 text-sm h-full transition-colors duration-200 font-medium flex-shrink-0"
									title="Yangilash"
								>
									{refreshing ? (
			...
			{timeModalOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
					<div className="fixed inset-0 backdrop-blur-sm bg-opacity-30" onClick={() => setTimeModalOpen(false)}></div>
					<div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-auto z-10">
						<div className="p-6">
							<div className="flex justify-between items-center mb-4">
								<h3 className="text-lg font-bold text-gray-800">
							...
							<div className="mb-6 text-sm text-gray-700 bg-blue-50 p-4 rounded-lg">
								Ushbu amalni tasdiqlashni xohlaysizmi?
								<br />
								<strong>{selectedUser?.fullName}</strong> - {modalType === "entry" ? "kirish" : "chiqish"} vaqti qayd etiladi.
							</div>
							<div className="flex justify-end gap-3">
								<button
									className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 text-sm font-medium transition-colors duration-200"
